...
async function fixture(customerId: number) { const product = await prisma.legoProduct.create({ data: { setNumber: `REFUND-HTTP-${randomUUID()}`, title: "Refund Product", theme: "TEST", ageRecommendation: "8+", pieceCount: 100, productListings: { create: {
            condition: "NEW", originalPrice: new Decimal("50.00"), salePrice: new Decimal("50.00"), currentStock: 10, active: true } } }, include: { productListings: true } }); productIds.push(product.id); const listing = product.productListings[0]; listingIds.push(listing.id); const order = await createOrder(customerId, { items: [{ productListingId: listing.id, quantity: 1 }] }); orderIds.push(order.id); const payment = await createPayment(order.id, { providerReference: `payment-${randomUUID()}` }); paymentIds.push(payment.id); return { listing, order, payment }; }
